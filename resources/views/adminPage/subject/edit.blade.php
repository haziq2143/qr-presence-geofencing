<x-admin-layout>
    <div class="w-full max-w-2xl mx-auto px-4 md:px-0 py-8">
        ...
        <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-lg shadow-lg p-8 mb-8">
            <h1 class="text-3xl font-bold text-white mb-2">Edit Mata Pelajaran</h1>
            <p class="text-green-100">Perbarui informasi mata pelajaran {{ $subject->subject }}</p>
        </div>

        <!-- Form Card -->
        <div class="bg-white rounded-lg shadow-lg p-8">
            <form action="/subjects/{{ $subject->id }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <!-- Subject Name -->
                <div>
                    <label for="subject" class="block text-sm font-semibold text-gray-700 mb-2">Nama Mata Pelajaran</label>
